...
<div class="card-container">
  <div class="card custom-card2">
    <form style="margin:10px" action="<?= base_url('retribusi_b_bayar/tambah_aksi') ?>" method="POST">
      <div class="card-body" style="background: linear-gradient(to bottom, #642EFE, #000066); color: white;">
        ...
        <div class="input-group input-group-sm">
          <input name="nama_objek" style="margin-bottom:-3px; height: 30px;" type="search" id="nama_objek"
            class="form-control" placeholder="Cari Nama Objek" aria-label="Search">
        ...
        $nomor_urut = 1;
        ?>
        <select multiple id="nama_objek_select" class="form-control" name="nama_objek_select" style="height: 300px;">
          <!-- Ubah tinggi sesuai kebutuhan Anda -->
          <?php foreach ($tb_npwrd as $obk) : ?>
          <option value="<?php echo $obk->nama_objek; ?>"
            ...
            <?php echo $nomor_urut . '. ' . $obk->nama_objek; ?></option>
          <?php
  ...
  <div class="card custom-card">
    ...
    <div class="card-body">
      ...
      <div class="row" style="margin: 1px">
        ...
        <div class="form-group col-md-3">
          ...
          <select id="status" class="form-control" name="status">
            <option value="" selected="">-- Select --</option>
            <?php foreach ($tb_petugas as $ptgs) : ?>
            <option value="<?php echo $ptgs->status; ?>"><?php echo $ptgs->status; ?></option>
            <?php endforeach; ?>
          </select>
        </div>
      ...
      <div class="form-group" style="margin-top: 5%; display: flex; justify-content: space-between;">
        <a type="button" class="btn btn-warning" onclick="history.back(-1)" name="btn_kembali"><i
            class="fa fa-arrow-left" aria-hidden="true"></i> Kembali</a>
        <a type="button" class="btn btn-info" href="<?= base_url('retribusi') ?>" name="btn_retribusi"><i
            class="fa fa-table" aria-hidden="true"></i> Lihat Data Retribusi</a>
